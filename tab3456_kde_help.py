...
def create_kde_tab(lbl, val, loading_id, plot_id):
    view= dcc.Tab(label= lbl, value=val,
            ...
            html.Div([  #####css-for-control panel
                dbc.Row([
                    dbc.Col(dcc.Loading(
                        id=loading_id,
                        type="circle", 
                        children=html.Div(dcc.Graph(id=plot_id, config={'scrollZoom': True, 'displayModeBar': False}),className="border p-2 my-2 rounded", style={"height": "530px"})
                    ), width=10),
                ])
...
def classify_cycle(series,comdty, out_df,lookback_prd, base_str, sum_first_n_base,  hike_threshold=50, dovish_threshold=-50, DEFAULT_WINDOW=20, DEFAULT_OUTLIER_K=2.5 ):
    """
    ...
    ##base_df= series
    base_df, comdty= process_help_calculation(comdty, out_df, base_str, lookback_prd, 20, DEFAULT_WINDOW, DEFAULT_OUTLIER_K )

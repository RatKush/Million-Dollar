...
default_ratios=[
    "OUT", "S3", "S6", "S12", "L3", "L3(II)", "L6(I)", "L6",
    "L12(I)", "L12(II)", "L12(III)", "L12", "D3", "D3(II)", "D6(I)", "D6", "D6(III)", "D6(IV)",
    "D12(I)", "D12(II)", "E3","E6(I)", "E6(II)", "1X L6- 2X L6(n+1)", "2X L6- 3X L6(n+1)", "1X L6- 2X L6(n+2)", "2X L6- 3X L6(n+2)"
]

def create_tab7_view(DEFAULT_CURVE_LENGTH):
    view_tab7= dcc.Tab(
    label='Matrix Filter',
    value='tab7',
    style={
                    ...
                    html.Div([
                        ...
                        dbc.Row([
                            ...
                            dbc.Col([
                                ...
                                html.Div(
                                    "Colors", id="color-title", # You can use a different title for clarity.
                                    className="fw-bold small px-2 py-1",
                                    style={
                                        "backgroundColor": "#1f2128", "borderBottom": "1px solid #3a3f4b",
                                        "borderTopLeftRadius": "6px", "borderTopRightRadius": "6px",
                                        "color": "#c0c4cc", "fontWeight": "500", "textAlign": "center",
                                        "padding": "8px 16px", "userSelect": "none","cursor": "default", "position": "relative"
                                    }
                                ),

                                ####### hidden by default ##### color scale ########
                                html.Div(
                                    id="colorscale-menu",
                                    style={
                                        "display": "none",
                                        "position": "absolute",
                                        "top": "calc(100% - 600px)",          # moves it upward relative to current column
                                        "right": "0px",          # aligns right edge with color button
                                        "transform": "translateY(-100%)",  # shifts it fully above the color titl
                                        "zIndex": "999",
                                    ...
                                    children=[
                                        dbc.RadioItems(
                                            id="color-scale-choice",
                                            options=[
                                                {"label": "Green → Red (Classic)", "value": "GR"},
                                                {"label": "Blue → Red (CoolHeat)", "value": "BR"},
                                                {"label": "Blue → Green (Soft)", "value": "BG"}
                                            ],
                                            value="BG",
                                            inline=False,
                                            style={"color": "#c0c4cc", "fontSize": "12px"}
                                        )
                                    ]
                                ),
                                ####### hidden by default ##### color scale ########
...
# str_data_3d= compute_3d_structure(out_df, all_strs, local_win=21, curve_length=20)
# latest_date = str_data_3d.index.get_level_values("Date").unique()[0]
# latest_df = str_data_3d.loc[(latest_date)]
...
custom_colorscale2 = [
    [0.0, 'rgb(100, 160, 255)'],  # Soft blue for 0% (e.g., lowest yield/volatility)
    [0.05, 'rgb(120, 175, 255)'], # Slightly lighter blue for 5%
    [0.10, 'rgb(140, 190, 255)'], # Clearly lighter for 10%
    [0.20, 'rgb(160, 205, 255)'], # Very light blue for 20%
    # --- Middle Section (Neutral transition) ---
    [0.50, 'rgb(220, 220, 230)'], # Pale grey-purple for midpoint (neutral, legible)
    # --- Reddish Extreme (High-distinction area) ---
    [0.80, 'rgb(255, 190, 190)'], # Very light red for 80%
    [0.90, 'rgb(255, 170, 170)'], # Clearly darker red for 90%
    [0.95, 'rgb(255, 150, 150)'], # Distinctly darker red for 95%
    [1.0, 'rgb(255, 130, 130)']  # Soft red for 100% (e.g., highest yield/volatility)
]

custom_colorscale3 = [
    [0.0, 'rgb(150, 215, 150)'],  # Strong light green for 0% (e.g., lowest yield/volatility)
    [0.05, 'rgb(160, 220, 160)'], # Slightly lighter green for 5%
    [0.10, 'rgb(170, 225, 170)'], # Clearly lighter for 10%
    [0.20, 'rgb(180, 230, 180)'], # Very light green for 20%
    # --- Middle Section (Neutral transition) ---
    [0.50, 'rgb(230, 230, 230)'], # Light grey for midpoint (neutral, legible)
    # --- Reddish Extreme (High-distinction area) ---
    [0.80, 'rgb(255, 190, 190)'], # Very light red for 80%
    [0.90, 'rgb(255, 170, 170)'], # Clearly darker red for 90%
    [0.95, 'rgb(255, 150, 150)'], # Distinctly darker for 95%
    [1.0, 'rgb(255, 130, 130)']  # Soft red for 100% (e.g., highest yield/volatility)
]

color_map = {
    "BG": custom_colorscale1,  # Green → Red (Classic)
    "BR": custom_colorscale2,  # Blue → Red (CoolHeat)
    "GR": custom_colorscale3   # Blue → Green (Soft)
}


def generate_heatmap(rounding, layer_df, color_pref):
    ...
    #color pick
    selected_colorscale = color_map[color_pref]
...
################# heatmap coloring  ##############################################################
import re
def parse_signed_value(s):
    """Safely extract signed numeric value from strings like '▲0.5', '▼1.2', or even malformed cells."""
    if pd.isna(s):            # Handles NaN / None
        return np.nan
    s = str(s).strip()
    if s == "":               # Handles empty string
        return np.nan
    matches = re.findall(r'[-+]?\d*\.?\d+', s)
    if not matches:           # Handles missing numbers
        return np.nan
    sign = -1 if  '▲' in s else 1
    try:
        return sign * float(matches[0])
    except ValueError:
        return np.nan
    
def color_heatmap(fig, arrowing, layer_df, color_pref): #initial value populating
    try:
        ...
        #  # --- 1️⃣ Extract numeric + apply sign from arrows
        if arrowing:
            layer_df['SignedValue'] = layer_df['Value'].apply(parse_signed_value)
        else:
            layer_df['SignedValue'] = pd.to_numeric(layer_df['Value'], errors='coerce')

        # --- 2️⃣ Convert MultiIndex Series to 2D DataFrame
        df_2d = layer_df['SignedValue'].unstack(level=0)
        ...
        if fig.data and isinstance(fig.data[0], go.Heatmap):
            ...
            fig.data[0].colorscale = selected_colorscale
